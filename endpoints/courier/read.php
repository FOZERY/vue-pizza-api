...
$sortBy = isset($_GET["sortBy"]) ? trim($_GET["sortBy"]) : null;
$searchBy = isset($_GET["searchBy"]) ? trim($_GET["searchBy"]) : null;
...
$stmt = $courier->read($sortBy, $searchBy, $limit, $offset);
$num = $stmt->rowCount();
...
if ($num > 0) {
    ...
    echo json_encode(array("message" => "Курьеры не найдены."), JSON_UNESCAPED_UNICODE);
}
